...
var base64ImageObject = {
  ...
  window.$chat.on('postMessage', function (type, data) {
    ...
    this.iframe_contanier.contentWindow.postMessage({type: type, data: data}, "*"); // 浼犻€佸浘鏂囨暟鎹�
  });
  ...
  function initCustomerServer(option) {
    this.outLine = false; // 鏄惁鍦ㄧ绾跨晫闈�
    this.openChat = false;//鏄惁闇€瑕佹墦寮€鑱婂ぉ绐楀彛
    this.settingObj = settingObj;
    this.baseUrl = option.openUrl;
    this.settingObj.openUrl = `${this.baseUrl}`; //鏈嶅姟鍣ㄥ湴鍧€鍔犺矾鐢�, 鑻ヤ笉浼犲叆鍒欒嚜鍔ㄨ幏鍙栧紩鍏ュ簲鐢ㄦ墍鍦ㄦ湇鍔″櫒鐨勫煙鍚�
    this.settingObj.domId = option.customerServerTip || 'customerServerTip'; //娴姩瀹㈡湇dom
    this.settingObj.insertDomNode = option.insertDomNode || 'body' // 鎻掑叆鐨勬爣绛�
    this.settingObj.pcIcon = option.pcIcon || base64ImageObject.pcIcon; // pcIcon 鐢佃剳绔鏈嶅浘鐗�
    this.settingObj.mobileIcon = option.mobileIcon || base64ImageObject.mobileIcon; // mobile 鎵嬫満绔鏈嶅浘鐗�
    this.settingObj.deviceType = option.deviceType || ''; // Mobile 鎵嬫満绔墦寮€
    this.settingObj.isShowTip = option.isShowTip; //  false闅愯棌 true 灞曠ず 瀹㈡湇鎮诞鎸夐挳榛樿灞曠ず
    this.settingObj.windowStyle = option.windowStyle || ''; // pc 绔墦寮€榛樿鏈€绮剧畝妯″紡锛宑enter灞呬腑妯″紡
    this.settingObj.kefuid = option.kefuid || 0; // 鎸囧畾瀹㈡湇锛岄粯璁ら殢鏈�
    this.settingObj.sendUserData = option.sendUserData || {}; // 鐢ㄦ埛淇℃伅锛岄粯璁ゆ父瀹�
    this.settingObj.productInfo = option.productInfo || {}; // 鎼哄甫浜у搧淇℃伅锛岄粯璁ょ┖
  //   this.settingObj.version = option.version || '',//鐗堟湰鍙�
    this.settingObj.uuid = option.uuid || ''; // 携带产品信息，默认空
    this.settingObj.name = option.name || 'youke'; // 携带产品信息，默认空
    this.settingObj.uid = option.uid || ''; // 携带产品信息，默认空
    this.settingObj.phone = option.phone || ''; // 携带产品信息，默认空
    this.settingObj.lang = option.lang || 'en-US'; // 携带产品信息，默认空
    this.appDom = null;
    this.initStatus = false;//鏄惁鍒濆鍖栬繃
  
    window.addEventListener('resize', () => {
        this.setMatchMedia();
    })
  
    // 鍒ゆ柇褰撳墠鐜涓嬬殑璁惧鏄痯c绔� || 绉诲姩绔�, 灏嗗鎴蜂俊鎭寕杞藉埌iframe鐨勯摼鎺ヤ笂
    this.setMatchMedia = () => {
  
        const matchMedia = window.matchMedia
        var deviceType = 'pc';
        // 鑷姩鍒ゆ柇鍚姩绔� pc 鎴栨槸 绉诲姩
        ...
        // 鑾峰彇瀹㈡湇瀹㈡埛鐩稿叧鍙傛暟
        let params = {
          uid: this.settingObj.uid,
          uuid: this.settingObj.uuid,
            ...
            name: this.settingObj.name,
            phone: this.settingObj.phone,
            lang: this.settingObj.lang,
        };
    ...
    // 鍒涘缓 鑱旂郴瀹㈡湇灏忓脊绐楁寜閽紙鐐瑰嚮鏃舵墦寮€鑱婂ぉ鐣岄潰锛�,鍒涘缓iframe瀹瑰櫒 骞跺皢iframe娣诲姞鑷砨ody涓�
    this.createCustomerServerContainer = () => {
        ...
        // 鍒涘缓瀹屾瘯鍚庯紝娣诲姞鏍峰紡锛屾牱寮忓彲浠ヤ粠澶栭儴浼犲叆
    ...
    // 璁剧疆鍒濆鍖栨牱寮忥紝鍖呮嫭iframe寮瑰鍒濆瀹氫綅锛屾湭璇绘秷鎭瓑
    ...
    this.loadwindow = () => {
        ...
        window.addEventListener("message", e => {
            ...
            // 鐩戝惉锛岃烦杞洖涓棿椤碉紝閲嶇疆outline锛堟潵鑷弽棣堟垚鍔熺晫闈級
    ...
    this.openChatWin = () => {
        ...
        this.iframe_contanier.contentWindow.postMessage({type: 'openCustomeServer'}, "*"); //閫氱煡iframe 鎵撳紑浜嗗鏈嶅脊妗�
        //鎵撳紑鑱婂ぉ绐椾簨浠�
    ...
    this.postProductMessage = (productInfo) => {
        this.iframe_contanier.contentWindow.postMessage({type: 'getImgOrText', productInfo: productInfo}, "*"); // 浼犻€佸浘鏂囨暟鎹�
    }
